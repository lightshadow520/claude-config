#!/usr/bin/env node
/**
 * 独立识图脚本 — 调用千问 VL 模型（阿里云百炼），OpenAI 兼容格式。
 *
 * 用法:
 ...
 *
 * 配置: 设置环境变量 VISION_API_KEY（阿里云百炼 API Key）
 */
...
function request(payload) {
  if (!API_KEY) throw new Error("请设置环境变量 VISION_API_KEY（阿里云百炼 API Key）");
...
async function main() {
  ...
  if (!imageSource) {
    ...
    console.error("");
    console.error("配置: export VISION_API_KEY=sk-你的千问API密钥");
    process.exit(1);
  }
  try {
