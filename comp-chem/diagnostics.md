...
同一项目/计算体系跨对话连续失败，且：
- S5（Sunk-Cost Guardian）已触发 ≥2 次，S6（Success Capture）未产出方案
- 换了 ≥2 种不同方向的修复后仍然失败 → 排除"参数微调能解决"的可能性
- 用户要求"深度分析""复盘""反思"
- 同一错误跨代码/跨服务器复现 → 排除环境问题

### 核心原则（来源：AgentErrorTaxonomy + CCRM + TraceSIR）

1. **根因 ≠ 表面症状**：OOM 的表面修复（降 ENCUT）可能与真实根因（真空层过大导致 FFT 网格爆炸）无关
2. **上下文污染效应（CCRM）**：每次失败尝试后的上下文记忆会污染下一次尝试的判断——复盘必须从零开始审查原始输入
3. **盲区优先检查**：多次失败中最稳定的不变参数恰恰最可能是根因

### 三层失败分类（AgentErrorTaxonomy）

| 层级 | 问题类型 | 计算化学示例 |
|------|---------|-------------|
| **Planning** | 输入准备阶段的错误 | POSCAR 构型不物理/原子重叠；INCAR 标签冲突；赝势泛函不匹配；初始磁矩设置错误 |
| **Execution** | 运行时的系统级失败 | OOM（并行布局/内存放大）；MPI 死锁；磁盘满；许可证耗尽；walltime 超 |
| **Response** | 成功运行但结果反常 | 能量不降反升；力不收敛到合理值；SCF 收敛到错误电子态；几何优化到非物理构型 |

**典型盲区**：反复调整 Execution 层参数（ENCUT/K 点/核心数）但根因在 Planning 层（初始磁矩设错）。复盘第一步就是检查失败是否跨层迁移。

### 深度复盘检查表

**A. 跨层检查**：
- [ ] 所有失败是否集中在 Execution 层？（如果是 → Planning 层可能有根本问题）
- [ ] 初始 POSCAR：原子间距是否 ≥0.5Å？晶胞是否合理？坐标是否有漂移？
- [ ] POTCAR：版本？与 INCAR 中的 LEXCH/GGA 是否匹配？排序是否与 POSCAR 一致？
- [ ] 磁矩：是否查过文献中的磁基态？MAGMOM 是否有多个可能值？是否试过非磁→磁矩重启？
- [ ] KPOINTS：是否手动生成（可能存在对称性 bug）？是否用 VASPKIT 生成？

**B. 盲区排查**：
- [ ] 列出所有失败尝试中**从未改变过的参数** → 这些是最可疑的
- [ ] 是否试过"极端保守"参数（ENCUT=200, KPOINTS=1x1x1, 1 核）先确认基本能跑？
- [ ] 是否试过同一组输入文件在另一个代码中验证？（如 VASP→CP2K、VASP→QE）
- [ ] 是否试过删除 WAVECAR/CHGCAR 从零开始？（防止前次失败污染电荷密度）

**C. 联网搜索**（强制，不可跳过）：
```
python websearch.py "<材料名> <晶面> VASP convergence difficulties"
python websearch.py "<错误关键词> <代码名> known issues"
python websearch.py "<材料名> computational study parameters INCAR"
```
搜索目标：该体系是否有已知的建模难点？同行用的什么参数？社区论坛是否有类似问题？

**D. 前提假设审查（最可能被忽略的）**：
- [ ] 晶胞选择：是否尝试过更大的超胞？当前尺寸是否太小导致周期性相互作用？
- [ ] Slab 模型：层数是否足够？底部几层是否固定？真空层是否真的需要那么大？
- [ ] 表面终止：是否有多种可能的终止面？当前选择的终止面是否最稳定？
- [ ] 吸附位点：是否考虑了所有高对称位点？初始位点选择是否合理？
- [ ] 电荷态：带电体系是否合理？背景电荷补偿是否正确？
- [ ] 泛函选择：PBE 是否适合该体系？是否需要 +U？是否需要范德华修正（DFT-D3/IVDW）？
...
▶ 模式分析
  共同主题：
  从不改变的参数（盲区）：
  跨层判断（Planning/Execution/Response）：
...
▶ 推荐新路径（按优先级）
  路径 A：[方案] — 置信度 [HIGH/MEDIUM/LOW]
  路径 B：[方案] — 置信度 [HIGH/MEDIUM/LOW]
  
▶ VERDICT
  [确信] 根因是 X → 建议执行路径 A，先用小体系验证
  [待验证] 可能是 X 或 Y → 建议先做对照实验排除
```

### 复盘后铁律

1. **报告给用户，禁止复盘后直接动手** — 用户可能知道 Claude 不知道的物理/化学约束
2. **一次只验证一个假设** — 改多个参数 = 不知道哪个起效
3. **小体系先行** — 用 1x1 表面/最小超胞/最低 K 点验证方案，5 分钟内出结果，确认有效再扩展
4. **记录每次验证结果** — 失败也是数据，持续丰富 Meso 层

### 与 S5/S6 的三角联动

```
S5 (Sunk-Cost Guardian)：止损 — ≥3 次失败 → 停
    │
    ├─ 找到方案 → S6 (Success Capture)：归档成功经验
    │
    └─ 找不到方案 → S7 (Deep Retrospective)：系统性深度复盘
                      │
                      ├─ 找到根因 → 验证 → S6 归档
                      └─ 找不到根因 → 输出"当前方法论局限"报告 → 建议换代码/方法
```
